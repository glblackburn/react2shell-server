...
| BUG-6 | Open | High | High | verify_scanner.sh Fails Due to Port Mismatch | Scanner verification script hardcodes port 5173 (Vite) but doesn't detect framework mode, causing failures when system is in Next.js mode (port 3000) | See details below |
...
See [Scanner Verification Table](docs/scanner_verification_table.md) for complete test results.
